...
-- Tạo key dựa trên ngày hoặc tháng
local os = os
local mode = "day" -- Thay "day" bằng "month" nếu muốn key theo tháng
local baseKey = "HungThinh" -- Key cơ bản

local validKey = ""
if mode == "day" then
    validKey = baseKey .. "-" .. os.date("250124") -- Key theo ngày
elseif mode == "month" then
    validKey = baseKey .. "-" .. os.date("2502") -- Key theo tháng
else
    error("Invalid mode. Use 'day' or 'month'.")
end
...
repeat
    ...
    userKey = io.read() -- Yêu cầu người dùng nhập key (CLI)
    if userKey ~= validKey then
        print("Invalid key. Please try again.")
    end
until userKey == validKey
