...
local Window = Library:CreateWindow('Menu : Hưng Thịnh') -- CREATE WINDOW

local Tab = { -- CREATE TABS
    Tab_1 = Window:addTab('#Trang Chính'),
    Tab_Setting = Window:addTab('#Tùy Chỉnh'),
    Tab_2 = Window:addTab('#Farm Chính'),
    Tab_SubFarm = Window:addTab('#Farm Phụ'),
    Tab_3 = Window:addTab('#Nhiệm Vụ'),
    Tab_Sea = Window:addTab('#Sea Event'),
    Tab_RaceV4 = Window:addTab('#Tộc V4'),
    ...
    -- Biến lưu mật khẩu
local MatKhau = "HungThinh" -- Bạn có thể thay đổi mật khẩu này
local DaNhapDung = false -- Biến kiểm tra nếu mật khẩu đã được nhập đúng

-- Hàm hiển thị giao diện nhập mật khẩu
local function NhapMatKhau()
    print("Vui lòng nhập mật khẩu để tiếp tục:")
    local nhap = io.read() -- Đọc mật khẩu từ người dùng

    if nhap == MatKhau then
        DaNhapDung = true
        print("Mật khẩu chính xác! Truy cập được cấp.")
    else
        print("Sai mật khẩu! Vui lòng thử lại.")
        NhapMatKhau() -- Gọi lại chính nó để người dùng thử lại
    end
end
...
if DaNhapDung then
    print("Bạn có thể truy cập vào hệ thống!")
    -- Thực hiện các chức năng khác ở đây
else
...
DisFarm = 30
Main_Setting:addTextbox("Phạm Vi Farm", DisFarm, function(Value)
    DisFarm = Value
end)
...
FastAttack = true
Main_Setting:addToggle('Đánh Nhanh (Melee Và Kiếm)', FastAttack, function(Value)
    FastAttack = Value
end)
spawn(function()
    while task.wait() do
...
Main_Setting:addToggle('Đánh Nhanh (Súng)', FastShot, function(Value)
    FastShot = Value
end)
spawn(function()
    while task.wait() do
...
Main_Setting:addToggle('Đánh Bằng Melee Người Chơi', AttackToPlayersNow, function(Value)
...
bringfrec = 250
Main_Setting:addTextbox("Phạm Vi Gom Quái(Mặc Định 250)", bringfrec, function(Value)
    bringfrec = Value
end)

local BringMobs = true
Main_Setting:addToggle("Gom Quái", BringMobs, function(Value)
    BringMobs = Value
end)
spawn(function()
    while task.wait() do
...
Skill_Setting:addToggle('Dùng Skill Z', _G.SkillZ, function(Value)
    _G.SkillZ = Value
end)

Skill_Setting:addToggle('Dùng Skill X', _G.SkillX, function(Value)
    _G.SkillX = Value
end)

Skill_Setting:addToggle('Dùng Skill C', _G.SkillC, function(Value)
    _G.SkillC = Value
end)

Skill_Setting:addToggle('Dùng Skill V', _G.SkillV, function(Value)
    _G.SkillV = Value
end)

Skill_Setting:addToggle('Dùng Skill F', _G.SkillF, function(Value)
    _G.SkillF = Value
end)

----------------------------------------------------//----------------------------------------------------

local Ability_Settings = Setting_Right:addMenu('#Cài Đặt Bản Năng')

local BusoHaki = true
Ability_Settings:addToggle("Haki Vũ Trang", BusoHaki, function(Value)
    BusoHaki = Value
end)
spawn(function()
    while wait() do
...
Ability_Settings:addToggle("Haki Quan Sát", KenHaki, function(Value)
    KenHaki = Value
end)
spawn(function()
    while wait() do
...
----------------------------------------------------//----------------------------------------------------
--// Misc Setting
local Misc_Settings = Setting_Right:addMenu('#Cài Đặt Khác')
...
local WeaponList = {"Melee","Trái Dứng","Kiếm","Súng"}
Config_Sea:addDropdown("Chọn Vũ Khí", SelectWeaponSeaFarm, WeaponList,function(weaponfunc)
...
--// LEVEL FARM
local Level_Farm = Farm_Left:addMenu('#Farm Cấp')

Level_Farm:addToggle('Farm Cấp Có Nhiệm Vụ', LevelFarmQuest, function(Value)
    LevelFarmQuest = Value
    _G.SelectMonster  = nil
    CancelTween(LevelFarmQuest)
end)

Level_Farm:addToggle('Farm Cấp Không Nhiệm vụ', LevelFarmNoQuest, function(Value)
...
----------------------------------------------------//----------------------------------------------------
--// OTHER FARM
local Other_Farm = Farm_Left:addMenu('#Farm Khác')
Other_Farm:addToggle("Random Xương", Auto_Trade_Bone, function(Value)
...
Other_Farm:addToggle('Farm Xương (Sea 3)', Farm_Bone, function(Value)
    Farm_Bone = Value
...
Other_Farm:addToggle('Farm Vật Chất Kì Dị (Sea 2)', Farm_Ectoplasm, function(Value)
...
Other_Farm:addToggle('Farm Xung Quanh', Nearest_Farm, function(Value)
    Nearest_Farm = Value
...
----------------------------------------------------//----------------------------------------------------
--// SELECT MONSTER
local SelectMonster_Farm = Farm_Left:addMenu('#Chọn Quái Farm')

SelectMonster_Farm:addDropdown("Chọn Quái", _G.SelectMonster, tableMon, function(Value)
    _G.SelectMonster = Value
end)

SelectMonster_Farm:addToggle('Tự Động Farm Quái Đã Chọn (Nhiệm Vị)', SelectMonster_Quest_Farm, function(Value)
...
SelectMonster_Farm:addToggle('Tự Động Farm Quái Đã Chọn (Không Nhiệm Vụ)', SelectMonster_NoQuest_Farm, function(Value)
...
KillPercent = 25
Mastery_Farm:addTextbox('Skill Dùng Khi %', KillPercent, function(Value)
    KillPercent = Value
end)

local MasteryType = {'Nhiệm Vụ', 'Không Nhiệm Vụ', 'Ở Gần', 'Xương', 'Ecto', 'Nữ Hoàng Bánh'}
SelectedMethodMastery = "Nhiệm Vụ"
Mastery_Farm:addDropdown('Chọn Cách Farm', SelectedMethodMastery, MasteryType, function(Value)
...
Mastery_Farm:addToggle('Auto Farm Súng Mastery', GunMastery_Farm, function(Value)
...
----------------------------------------------------//----------------------------------------------------
--// BOSS FARM
local Boss_Farm = Farm_Right:addMenu('#Farm Boss')
...
Boss_Farm:addButton('Làm Mới Boss', function()
    local NewBossList = {}
    for i,v in pairs(game.ReplicatedStorage:GetChildren()) do
        if table.find(BossNameStorage, v.Name) then
...
Boss_Farm:addToggle('Tự Động Farm Boss (Nhiệm Vụ)', AutoFarmBossQuest, function(Value)
...
Boss_Farm:addToggle('Tự Động Farm Boss (Không Nhiệm Vụ)', AutoFarmBossNoQuest, function(Value)
...
Other_Subs_Farm:addToggle('Tự Động Farm Nhà Máy', AutoFactory, function(Value)
    AutoFactory = Value
...
Other_Subs_Farm:addToggle('Tự Động Farm Hải Tặc', AutoPirateCastle, function(Value)
...
--// MATERIAL FARM
local Material_Farm = Subs_Farm_Right:addMenu('#Farm Vật Liệu')
if First_Sea then
...
Material_Farm:addDropdown('Chọn Vật Liệu', SelectMaterial, MaterialList, function(Value)
    SelectMaterial = Value
end)

Material_Farm:addToggle('Tự Động Farm Vật Liệu', Auto_Farm_Material, function(Value)
...
----------------------------------------------------//----------------------------------------------------
--// CHEST FARM
local Chest_Farm = Subs_Farm_Right:addMenu('#Farm Rương')
...
Chest_Farm:addToggle('Tự Động Farm Rương(Dừng Lại Nếu có ITEM)', _G.GrabChested, function(Value)
...
Chest_Farm:addToggle('Tự Động Farm Rương + Đổi Sever (Dừng Lại Nếu Có ITEM)', _G.GrabChestedHop, function(Value)
...
local World_Quest = Quest_Left:addMenu('#Thế Giới')
...
local Teleport_World = TeleportStatus_Left:addMenu('#Dịch Chuyển Thế Giới')
Teleport_World:addButton("Bay Về Sea 1", function()
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelMain")
end)
    
Teleport_World:addButton("Tốc Biến Đến Sea 2", function()
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelDressrosa")
end)
    
Teleport_World:addButton("Bay Về Sea 3", function()
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelZou")
end)

----------------------------------------------------//----------------------------------------------------
--// Island Teleport
local Island_Teleport = TeleportStatus_Left:addMenu('#Dịch Chuyển Đảo')
if First_Sea then
    IslandCheck = {
        "Start Island";
        "Marine Start";
...
Island_Teleport:addDropdown("Chọn Đảo", SelectedIsland, IslandCheck, function(Value)
    SelectedIsland = Value
end)

Island_Teleport:addButton("Dịch Chuyển Đến Đảo", function()
...
Island_Teleport:addToggle("Bypass Dịch Chuyển", BypassTeleport, function(Value)
...
local Status_Player = TeleportStatus_Right:addMenu('#Trạng Thái Người Chơi')
...
spawn(function()
    while wait() do
        RaceStatus:Refresh("Tộc : "..tostring(game:GetService("Players").LocalPlayer.Data.Race.Value))
        PointStatus:Refresh("Điểm Kĩ Năng: "..tostring(game:GetService("Players").LocalPlayer.Data.Points.Value)..' Điểm')
        StatusFunc1:Refresh("Melee : "..tostring(game:GetService("Players").LocalPlayer.Data.Stats.Melee:WaitForChild("Level").Value)..' Điểm Còn Lại')
        StatusFunc2:Refresh("Phòng Thủ : "..tostring(game:GetService("Players").LocalPlayer.Data.Stats.Defense:WaitForChild("Level").Value)..' Điểm')
        StatusFunc3:Refresh("Kiếm : "..tostring(game:GetService("Players").LocalPlayer.Data.Stats.Sword:WaitForChild("Level").Value)..' Điểm')
        StatusFunc4:Refresh("Súng : "..tostring(game:GetService("Players").LocalPlayer.Data.Stats.Gun:WaitForChild("Level").Value)..' Điểm')
        StatusFunc5:Refresh("Trái Dứng : "..tostring(game:GetService("Players").LocalPlayer.Data.Stats["Demon Fruit"]:WaitForChild("Level").Value)..' Điểm')
    end
end)
----------------------------------------------------//----------------------------------------------------
--// Set Status
local Set_Status = TeleportStatus_Right:addMenu('#Cài Đặt Trạng Thái ')
SelectPoint = 1
Set_Status:addTextbox("Set Điểm", SelectPoint, function(Value)
    SelectPoint = Value
end)
...
Set_Status:addToggle("Phòng Thủ", Defensef, function(Value)
    Defensef = Value
...
Set_Status:addToggle("Kiếm", Swordf, function(Value)
    Swordf = Value
...
Set_Status:addToggle("Súng", Gunf, function(Value)
    Gunf = Value
...
Set_Status:addToggle("Trái Dứng", DevilFruitf, function(Value)
    DevilFruitf = Value
...
local RaceFragShop = Shop_Left:addMenu('#Xương - Ecto - Fragment')
...
DevilShop:addToggle("Dịch Chuyển Đến Trái", _G.TeleportFruit, function(Value)
...
----------------------------------------------------//----------------------------------------------------
--// Gun Shop
local GunShop = Shop_Right:addMenu('#Súng)
...
----------------------------------------------------//----------------------------------------------------
--// Accessoreies Shop
local AccessoriesShop = Shop_Right:addMenu('#Phụ Kiện')
...
----------------------------------------------------//----------------------------------------------------
--// MISC RIGHT
local Misc_2 = Misc_Left:addMenu('#Khác')
...
Misc_2:addToggle('Vô Hạn Năng Lượng', InfiniteAbility, function(Value)
...
Misc_2:addToggle('Vô Hạn Geppo', InfiniteGeppo, function(Value)
    InfiniteGeppo = Value
end)
spawn(function()
    while task.wait(1) do
...
Misc_2:addToggle('Vô Hạn Soru', InfiniteSoru, function(Value)
    InfiniteSoru = Value
end)
spawn(function()
    while task.wait(1) do
...
WalkOnWater = true
Misc_2:addToggle('Đi Trên Nước', WalkOnWater, function(Value)
    WalkOnWater = Value
end)
spawn(function()
    while task.wait(1) do
...
Misc_2:addToggle('Màn Hình Trắng', _G.White_Screen, function(Value)
...
----------------------------------------------------//----------------------------------------------------
local Misc_1 = Misc_Left:addMenu('#Khác')
...
Misc_1:addButton('Nhập X2 Code', function()
...
Misc_1:addButton("Gia Nhập Hải Tặc", function()
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("SetTeam","Pirates") 
end)
    
Misc_1:addButton("Gia Nhập Hải Quân", function()
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("SetTeam","Marines") 
end)

Misc_1:addButton("Luôn Là Ngày", function()
    while game:GetService("RunService").Heartbeat:wait() do
        game:GetService("Lighting").ClockTime = 12
    end
end)

Misc_1:addButton("Xoá Lava", function()
...
Misc_1:addButton("Màu Haki", function()
...
ESP_Section:addToggle('ESP Rương', ChestESP, function(Value)
    ChestESP = Value
end)
spawn(function()
    while wait() do
...
ESP_Section:addToggle('ESP Trái Dứng(Màu Đỏ)', _G.ESPDF, function(Value)
    _G.ESPDF = Value
end)
spawn(function()
    while wait() do
...
ESP_Section:addToggle('ESP Trái Cây (Màu Vàng)', FruitESPe, function(Value)
    FruitESPe = Value
end)
spawn(function()
    while wait() do
...
ESP_Section:addToggle('ESP Hoa', FlowerESP, function(Value)
    FlowerESP = Value
end)
spawn(function()
    while wait() do
...
ESP_Section:addToggle('Định Vị Đảo', IslandESP, function(Value)
    IslandESP = Value
end)
spawn(function()
    while wait() do
...
ESP_Section:addToggle('Định Vị NPC', NpcESP, function(Value)
    NpcESP = Value
end)
spawn(function()
    while wait() do
...
ESP_Section:addToggle('Định Vị Người Bán Kiếm Legendary ', LADESP, function(Value)
    LADESP = Value
end)
